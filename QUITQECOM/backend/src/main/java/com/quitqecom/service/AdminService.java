...
public class AdminService {
    ...
    getAllCustomers(int page, int size, String search) {

        Pageable pageable = PageRequest.of(page, size);
        Page<Customer> pages;
        if(search != null && !search.isBlank()) {
            pages = customerRepository.findByFirstNameContainingIgnoreCaseOrLastNameContainingIgnoreCaseOrUser_EmailContainingIgnoreCaseOrUser_UsernameContainingIgnoreCase(
                                    search,
                                    search,
                                    search,
                                    search,
            ...
            pages = customerRepository.findAll(pageable);
        }

        return customerMapper.mapEntityToDto(pages);
    }
    ...
    public AdminProductPageRespDto getAllProducts(
        ...
        Page<Product> pages = productRepository.searchProducts(search, category, seller, inStock, pageable);

        return adminProductMapper.mapEntityToDto(pages);
    }
